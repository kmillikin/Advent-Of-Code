...
VARIABLE STACKS
SIZE 10 * ALLOT  ( 9 PACKED STRINGS WITH AT LEAST SIZE-1 CHARACTERS. )
...
: RUN ( -- )
    0 10 SET-LENGTH  ( USE A 10TH SCRATCH STACK. )
    BEGIN
	?NOT-EOF
    WHILE
	    GET-NEXT-LINE DROP
	    PARSE-LINE >R       ( NUM SRC R:DST )
	    OVER SWAP 10        ( NUM NUM SRC 10 )
	    PERFORM-MOVE 10 R>  ( NUM 10 DST )
	    PERFORM-MOVE
    REPEAT
;
