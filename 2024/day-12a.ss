(define input
  (call-with-input-file "day-12.input"
    (lambda (ip)
      (let loop ([line (get-line ip)] [rows '()])
        (if (eof-object? line)
            (list->vector (reverse rows))
            (loop (get-line ip)
              (cons (list->vector (string->list line)) rows)))))))

(define (neighbors c row col)
  (filter (lambda (p)
            (let ([row (car p)] [col (cdr p)])
              (and (>= row 0)
                   (< row (vector-length input))
                   (>= col 0)
                   (< col (vector-length (vector-ref input row)))
                   (char=? c (vector-ref (vector-ref input row) col)))))
  (list
    (cons (1- row) col)
    (cons (1+ row) col)
    (cons row (1- col))
    (cons row (1+ col)))))

;; Flood fill starting at a position with a character.  Mark the cells
;; in the input with dot characters.
(define (flood-fill row col)
  (let ([c (vector-ref (vector-ref input row) col)]
        [visited (vector-map vector-copy input)])
    (let recur ([worklist (list (cons row col))]
                [perimeter 0]
                [area 0])
      (if (null? worklist)
          (values visited perimeter area)
          (let ([row (car (car worklist))]
                [col (cdr (car worklist))])
            (if (char=? #\. (vector-ref (vector-ref visited row) col))
                (recur (cdr worklist) perimeter area)
                (begin
                  (vector-set! (vector-ref visited row) col #\.)
                  (let ([ns (neighbors c row col)])
                    (recur (append ns (cdr worklist))
                      (+ perimeter (- 4 (length ns)))
                      (1+ area))))))))))

(let loop ([row 0] [col 0] [price 0])
  (cond
    [(= row (vector-length input))
     (display price)
     (newline)]
    [(= col (vector-length (vector-ref input row)))
     (loop (1+ row) 0 price)]
    [(char=? #\. (vector-ref (vector-ref input row) col))
     (loop row (1+ col) price)]
    [else
      (let-values ([(result perimeter area) (flood-fill row col)])
        (set! input result)
        (loop row (1+ col) (+ price (* perimeter area))))]))
